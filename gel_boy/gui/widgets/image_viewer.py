...
class ImageViewer(QGraphicsView):
    ...
    def update_display(self) -> None:
        ...
        if self.current_image.mode == 'RGB':
            ...
            qimage = QImage(data, self.current_image.width, self.current_image.height, 
                          self.current_image.width * 3, QImage.Format.Format_RGB888)
        elif self.current_image.mode == 'L':
            data = self.current_image.tobytes('raw', 'L')
            qimage = QImage(data, self.current_image.width, self.current_image.height,
                          self.current_image.width, QImage.Format.Format_Grayscale8)
        else:
